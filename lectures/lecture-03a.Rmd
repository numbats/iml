...
.question-box.monash-blue2[Why is a linear model not appropriate for this data?]
...
ggplot(simcredit_fit, aes(x=balance, y=default_bin)) +
  ...
  theme_minimal(base_size=18) 
```
.monash-orange2[Orange points] are fitted values, $\hat{y}_i$. Black points are observed response, $y_i$. (Residual is the difference between observed and predicted.) To generate categorical predictions, round the fitted values. 
]
]
...
$\hat{\beta}_1 =$ `r broom::tidy(logistic_fit)[1, 3]`
...
<br>
<center>
.info-box[Note: Residuals not typically useful.]
<center>

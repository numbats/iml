...
Repeat optimisation to estimate $\phi_{jk}$, with additional constraint that $\sum_{{j=1}^p, k<k'} \phi_{jk}\phi_{jk'} = 0$ (next vector is orthogonal to previous eigenvector).
...
.info-box[.monash-orange2[Think:] How do we know how many principal components to choose?]
...
- Classical MD similar results to PCA

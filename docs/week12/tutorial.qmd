...
It still divides the data more messily than anticipated. Cluster 1 though does have small values on all variables. This is the low risk group. Cluster 2 seems to have high values on Recreation and somewhat high on Social but low values on other variables. So it might be considered a cluster of high recreational risk takers. Now your turn to describe the other clusters.

::: unilur-solution
Cluster 3 has high values on Health and Recreational and mostly high on all variables. These are the general risk takers.

Cluster 4 has high values on Safety but relatively low on other variables.

Cluster 5 is generally low values on all variables, but maybe slightly higher than cluster 1.

Cluster 6 has high values on Health. 

Cluster 7 has relatively low values on all variables, but slightly higher on Safety.

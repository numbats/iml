...
footer: "ETC3250/5250 Lecture 11"
format:
  revealjs:
    multiplex: false
    slide-number: c/t
...
## Confusion tables: comparing results [(1/4)]{.smallest}
...
+ Both have reasonable support for 2 clusters
+ M1 has support for 3 clusters while M2 supports 4
+ M2 has support for 6 clusters and M1 support 9
...
- Is the 2-cluster solution from M1 and M2 the same?
- Two clusterings can be compared with a confusion matrix, similarly to supervised classification.
- However, here we are comparing methods to each opther rather than to the truth
...
## Confusion tables: comparing results [(2/4)]{.smallest}
...
x <- (runif(20)-0.5)*4
y <- x
df0 <- data.frame(x1=scale(c(rnorm(50, -3), rnorm(50, 3), x)),
                   x2=scale(c(rnorm(50, -3), rnorm(50, 3), y)))

df0_mc <- Mclust(df0, G = 2)

df0 <- df0 |> mutate(cl = as.factor(df0_mc$classification))


ggplot(data=df0, aes(x1, x2, color = cl)) + geom_point() +
...
df0 <- df0 |> mutate(cl = as.factor((2-df0_mc$classification) + 1))
...
## Confusion tables: comparing results [(3/4)]{.smallest}

:::: {.columns}
::: {.column}

We compare the 2-cluster solutions
...
::: {.fragment}
Do we now think these clusters are similar or different?
:::
...
## Confusion tables: comparing results [(4/4)]{.smallest}
...
- Average Linkage: 2 or 5 clusters?
- Wards Linkage: 2, 3 or 5 clusters?
...
Comparing the 5 cluster solutions (note reordering)
...
$k$-means is always drawing circles 
...
- Means: which variables differentiate that variable from another
- Standard deviations: how tightly packed is that cluster 
- Variances and covariances: but unwieldy to print all
- Cluster size

:::
::: {.column width=70% style="font-size: 70%;" .fragment}

Wards linkage hierarchical clustering
...
- [Model in the data space]{.monash-blue2}: 
    - Colour observations by their cluster label, and use multivariate plots: Scatterplot matrix, parallel coordinates and tours
...
Shading represents 1 standard deviation above and below the mean
...
## Model-in-the-data-space [(1/4)]{.smallest}
...
## Model-in-the-data-space [(2/4)]{.smallest}
...
## Model-in-the-data-space [(3/4)]{.smallest}
...
## Model-in-the-data-space [(4/4)]{.smallest}
...
A set $S\subset \mathbb{R}^d$ is convex if for any $x\in S$, $y\in S$ and $\alpha\in [0, 1]$ then $\alpha x + (1-\alpha)y\in S$ 


::: {.fragment}

Convex hulls are a common method for visually displaying the region in the data space corresponding to each cluster. 
...
It avoids making any assumption about the shapes.
...
<center>
![](https://dicook.github.io/mulgar_book/gifs/penguins_chull.gif){width=500}
</center>
...
- PCA, but not PCA
- Perhaps, projection pursuit (guided tour)
...
Lets look at the `c1` data from the `mulgar` package
...
Your clustering model is 'right', if it was useful 
...
- Are the subsets useful, e.g. if you market differently to the different subsets do you get more purchases than marketing the same way to all.
...
- [Computational and statistical models for a categorical response]{.monash-blue2}: logistic regression, LDA, decision trees, forests, support vector machines, kNN, neural networks
- [Discovering class labels by clustering]{.monash-blue2}: $k$-means, hierarchical, model-based, self-organising maps
...
- *Algorithms for model fitting*: Logistic regression, neural networks and SVM $\longrightarrow$ ETC3555/5555    
- *Full range of neural networks*: how to prevent overfitting, convolutional, recurrent, word embeddings $\longrightarrow$ ETC3555/5555
...
:::: {.columns  }
::: {.column}
End of semester exam counts for 60\% of the module grade 

<br>

::: {.fragment}

You will have 2hrs and 10 mins to answer 6 Questions

:::

::: {.fragment}

This is a closed-book eExam with Specifically permitted items
...
+ Physical Calculator
+ Double Sided sheet of A4 (must be physical)
...
The exam will be in supervised exam conditions
...
3 questions, each made up of approx 4 short essay questions

e.g.

::: {.incremental}

+ Here is the output of this model in R, what is it telling us?
+ What do we learn about the structure in our data from this image?
+ Why is this bit of methodology important for this model?
+ What do you need to do to this data to fit this model?
+ What's the difference between this model and this model?
+ What complications are there fitting this model in stead of this model?
+ What are the benefits of using this methodology over that methodology? 
+ Based on this output which model would you choose?

:::

::: {.fragment}

Short typed answers in eAssessment platform 

:::
:::
::: {.column .fragment}

3 hybrid questions, longer and made up of stages

e.g.

::: {.incremental}

+ Implement one iteration of this algorithm on this toy data
+ Define this model
+ Sketch this model
+ Explain how this algorithm works and why its preferable to another algorithm
+ Explain the process with which you would learn these parameters or hyperparameters
...
Exam consultation Thursday 12th April 9:30am-11:30am

...
d. How many parameters need to be estimated for the VVI model with 8 clusters? (Use the mixture model equation for calculating this, that is, how $\Sigma_k$ would change, or not, between clusters.) Compare this to the number of observations, and explain why the model cannot be estimated.

::: unilur-solution

VVI means the volume and shape are variable, but there is no covariance between variables, so they are all diagonal. It can be written as  $\lamba_k A_k$.

$n=507, p=11, G=8$

- $\widehat{\mu}$: $8\times 11= 88$
- $\widehat{\Sigma}$: $8\times 11= 88$
- $\pi$ (mixing proportion): $8-1 = 7$

The total number of parameters is 183.

If we were to use the decomposition formula, then for $\widehat{\Sigma}$ we would have  $8\times 8\times 11= 704$. Total parameters are 799. We only have 507 observations, so this model cannot be estimated.
:::

d. Fit just the best model, and extract the parameter estimates. 
...
```{r}
#| eval: false
animate_xy(music_som1_map_data[,1:4],

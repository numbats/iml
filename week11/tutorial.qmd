...
Had to run this in the week 11 folder in the terminal in R
cd C:\Users\jjew0003\Documents\Monash Teaching\ETC3250-5250\2025\week11
quarto install extension ginolhac/unilur

or 
cd C:\Users\jjew0003\Documents\Monash_Yr1\ETC3250\S12025\S12025\week11
quarto install extension ginolhac/unilur
-->
...
This exercise is motivated by [this blog post](https://medium.com/data-science/unsupervised-anomaly-detection-on-spotify-data-k-means-vs-local-outlier-factor-f96ae783d7a7) on using $k$-means to identify anomalies.
...
```{r}
# https://medium.com/data-science/unsupervised-anomaly-detection-on-spotify-data-k-means-vs-local-outlier-factor-f96ae783d7a7
...
library(detourr)
set.seed(645)
detour(spotify_tf[,5:15], 
       tour_aes(projection = 1:11)) |>
...
- $\lambda_k, \widehat{A}_k$ (when both are variable we can combine and think of estimating 7 $11\times 11$-D diagonal matrices): $7\times 11= 77$
- $D$ (one $11\times 11$-D matrix, upper and lower triangles the same): $\sum_{i=1}^7 i = 28$
- $\pi$ (mixing proportions): $7-1 = 6$

For 7 clusters 188 parameters are estimated. 

- $\widehat{\mu}_k$ (8 11-D means): $8\times 11= 88$
- $\lambda_k, \widehat{A}_k$ (when both are variable we can combine and think of estimating8 $11\times 11$-D diagonal matrices): $8\times 11= 88$
- $D$ (one $11\times 11$-D matrix, upper and lower triangles the same): $\sum_{i=1}^8 i = 36$
- $\pi$ (mixing proportions): $8-1 = 7$

For 8 clusters 219 parameters are used. 
...
#x11()
animate_xy(spotify_tf[,5:15], col=spotify_mc$classification)
```
:::

...
library(gridExtra)
p1 <- ggplot(Auto, aes(x=horsepower, y=mpg)) + geom_point(alpha=0.3) + ggtitle("Full data")
p2 <- Auto %>%
  sample_n(size = n(), replace = TRUE) %>% ggplot(aes(x=horsepower, y=mpg)) + geom_point(alpha=0.3) + ggtitle("Bootstrap sample 1")
p3 <- Auto %>%
  sample_n(size = n(), replace = TRUE) %>% ggplot(aes(x=horsepower, y=mpg)) + geom_point(alpha=0.3) + ggtitle("Bootstrap sample 2")
...
mse.fn <- function(d, index) {
  glm.fit <- glm(mpg ~ horsepower, 
               data=d[index,])
  # MSE on all observations
  return(m = mse(glm.fit, d[-unique(index),]) )
}
m <- NULL
for (i in 1:1000) {
  idx <- sample(nrow(Auto), replace = TRUE)
...
```{r}
# Foor coonstructing a confidence interval

---
title: "ETC3250/5250: Hierarchical clustering"
...
```{r out.width="100%", fig.width=4, fig.height=4}
df$cl11 <- factor(c(2,2,2,1,1,1,1,1,1,1,1,1))
...
What is the .orange[distance] between the new .orange[cluster (a, b,c)] and all of the other observations?
...
kable(d) %>%
  ...
  column_spec(1, color = "white", background = "#3F9F7A") %>%
  column_spec(1:12, border_right=TRUE, width="1cm") %>%
  column_spec(2:4, color="#CA6627") %>%
  row_spec(1:3, color="#CA6627")

```
]]]
.column[.pad50px[.content[

.orange[Distance (linkage) between (a,b,c) and d]

Single: 3.6 or 5.1 $\rightarrow$ .orange[3.6] <br>
Complete: 3.6 or 5.1 $\rightarrow$ .orange[5.7] <br>
Average: (3.6 + 3.6 + 5.1)/3 = .orange[4.1] <br>
Centroid: .orange[4.1] <br>
mean of (a,b,c) is (13.3, 7.7) <br>
          mean of d (15, 4) <br>
          $\sqrt{(13.3-15)^2 + (7.7-4)^2}$ <br>
Wards: Your turn to calculate it `r set.seed(1) ;emo::ji("shrug")`

```{r eval=FALSE}
# For computing centroid
# df %>% filter(lbl %in% c("d", "e", "f", "g", "h", "i", "j", "k", "l")) %>% 
df %>% filter(lbl %in% c("a", "b", "c")) %>% 
  select(x1, x2) %>% 
  summarise_all(mean)
```
]]]
...
kable(d2) %>%
  ...
  column_spec(2, color="#CA6627") %>%
  row_spec(1, color="#CA6627")
```
]]]
.column[.pad50px[.content[
...
.pull-left[
<iframe src="http://iml.numbat.space/lectures/week11/cluster_ward.html" width="400" height="500" scrolling="yes" seamless="seamless" frameBorder="0"> </iframe>
]
.pull-right[
<iframe src="http://iml.numbat.space/lectures/week11/cluster_average.html" width="400" height="500" scrolling="yes" seamless="seamless" frameBorder="0"> </iframe>
]

---

layout: false
# `r emo::ji("woman_technologist")` Made by a human with a computer

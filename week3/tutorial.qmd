...
cd C:\Users\jjew0003\Documents\Monash Teaching\ETC3250-5250\S12026\week3
...
a. Write a projection matrix which would generate a 2D projection where the first data projection has variables 1 and 4 combined equally, and the second data projection has variables 2 and 5 combined with variable 5 having greater weight.
...
Note that there are several solutions for the second column, but this is one that satisfies the requirements of the question.
:::

b. Why can't the following matrix considered a projection matrix?
...
a. Explain what is meant by plotting the 'data-in-the-model-space' and plotting the 'model-in-the-data-space'


::: unilur-solution

data-in-the-model-space corresponds to plotting the model and trying to visualise the data in this space. This is convenient as the model usually produces a low dimensional output which we can plot. Examples of this would be plotting the probabilities or predictions estimated by a model. However, a plot like this doesn't provide an explanation of why the model is behaving the way that it is.

model-in-the-data-space corresponds to plotting the data and trying to visualise the model on top of the data. This is often harder as the data is general high dimensional, however it can provide a lot more information about why a model is or is not performing well



:::

b. Describe one advantage and one disadvantage of using a scatterplot matrix to visualise moderate to high dimensional data

::: unilur-solution

Advantage 

+ As long as the dimension is not too big scatterplot matrix allows us to view all pairwise relationships in the data in one plot

Disadvantage 

+ Can't plot more than a handful fo variables together
+ Only looks at the data pairwise so can't see relationships in more than 2-dimensions


:::

c. Describe one advantage and one disadvantage of using a parallel coordinates plot to visualise moderate to high dimensional data

::: unilur-solution

Advantage 

+ Allows for plotting more variables on a single page than scatterplot matrix

Disadvantage 

+ The order that the variables matters
+ It can be hard to discern patterns between variables that are not plotted next to each other


:::

d. Below is a screenshot taken from a tour of the penguins data. Which two variables are projected the most in this view of the data?

![](../images/penguins_tour_1.png){width=700}

::: unilur-solution

In the bottom left corner we see that `bm` and `fl` have the longest arrows which suggests that these two variables are projected the most in this view

:::

e. Below is another screenshot taken from a tour of the penguins data. In this view of the tour we see evidence that there are at least two clusters in the data. Which variable appears to be driving the clustering structure that we see here the most?

![](../images/penguins_tour_2.png){width=700}

::: unilur-solution

The clustering can be seen most along the vertical axis, with a slight tilt to the right. We see from the bottom left that variable `bm` is encoded to a large degree in this direction and so it is this variable that appears to be driving the clustering we see here.

:::

#### 3. Detecting clusters
...
#### 5. Principal components analysis on the simulated data

🧐 For data sets `d2` and `d3` from Q4 what would you expect would be the number of PCs suggested by PCA?
...
#### 6. PCA on cross-currency time series

In Moodle, under Learning -> Getting Started you will find a data folder. Download this folder and you will it that contains the `rates_Nov19_Mar20.csv.csv` data.

The `rates_Nov19_Mar20.csv.csv` data has 152 currencies relative to the USD for the period of Nov 1, 2019 through to Mar 31, 2020. Treating the dates as variables, conduct a PCA to examine how the cross-currencies vary, focusing on this subset: ARS, AUD, BRL, CAD, CHF, CNY, EUR, FJD, GBP, IDR, INR, ISK, JPY, KRW, KZT, MXN, MYR, NZD, QAR, RUB, SEK, SGD, UYU, ZAR. 

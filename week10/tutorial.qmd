...
Had to run this in the week 10 folder in the terminal in R
cd C:\Users\jjew0003\Documents\Monash Teaching\ETC3250-5250\2025\week10
quarto install extension ginolhac/unilur

or 
cd C:\Users\jjew0003\Documents\Monash_Yr1\ETC3250\S12025\S12025\week10
quarto install extension ginolhac/unilur
-->
...
Yes, for example "Free Bird" and "Sparkle" could be found by simply examining the `duration_ms`.
:::
...
```{r}
x <- cluster.stats(dist(spotify_tf[,5:15]), spotify_km3$cluster)
...
spotify_assign_df |>
  select(name:`artist_popularity`, `11`) |>
  pivot_longer(danceability:artist_popularity,
               names_to="var", values_to="value") |>
...
You can see form the dendrogram that there is a good 6 cluster solution though. You would just need to first split the data in two, then take the 4 cluster solution on the left and the 2 cluster soplution on the right
